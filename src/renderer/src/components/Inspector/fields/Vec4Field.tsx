...
interface Vec4FieldProps {
  uniform: ShaderUniform<'vec4'>
  onChange: (value: [number, number, number, number]) => void
}

const LABELS = ['X', 'Y', 'Z', 'W'] as const

export function Vec4Field({ uniform, onChange }: Vec4FieldProps): React.JSX.Element {
  const handleComponentChange = useCallback(
    (index: number, e: ChangeEvent<HTMLInputElement>) => {
      const parsed = parseFloat(e.target.value)
      if (isNaN(parsed)) return
      const next: [number, number, number, number] = [...uniform.value] as [
        number,
        number,
        number,
        number
