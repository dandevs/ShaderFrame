...
interface Vec2FieldProps {
  uniform: ShaderUniform<'vec2'>
  onChange: (value: [number, number]) => void
}

const LABELS = ['X', 'Y'] as const

export function Vec2Field({ uniform, onChange }: Vec2FieldProps): React.JSX.Element {
  const handleComponentChange = useCallback(
    (index: number, e: ChangeEvent<HTMLInputElement>) => {
      const parsed = parseFloat(e.target.value)
      if (isNaN(parsed)) return
      const next: [number, number] = [...uniform.value] as [number, number]
      next[index] = parsed
      onChange(next)
    },
